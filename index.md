...
<section class="section section-stone"><div class="wrap"><div class="section-intro"><span class="eyebrow">Representative situations</span><h2>Start with the condition you recognize.</h2></div><div class="scope-grid">{% assign home_placement = site.data.featured_placements | where: 'id', 'home-representative-scopes' | first %}{% for slot in home_placement.slots %}{% assign scope = site.data.scopes | where: 'id', slot.scope | first %}{% if scope %}{% include scope.html scope=scope %}{% endif %}{% endfor %}</div></div></section>
